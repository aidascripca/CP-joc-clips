...
import random
import sys
import clips  # Importam biblioteca pentru CLIPS
...
try:
    ...
    print("Asigura-te ca fisierul agent.clp exista in acelasi folder cu acest script.")
    sys.exit()
...
while running:
    ...
    if not game_over:
        if timer_tranzitie_nivel > 0:
            ...
            if stamina_epuizata:
                ...
                for sac in lista_saci_brate:
                    if sac == "sac_gold":
                        penalizare_greutate += 1
                    elif sac == "sac_diamond":
                        penalizare_greutate += 2.5
            ...
            if cooldown_timer == 0:
                ...
                # Extragem decizia AI-ului
                tinta_x = None
                for fact in env.facts():
                    if fact.template.name == "comanda":
                        actiune = fact[0]
                        if actiune in ["prindere_libera", "mergi_la_platforma", "evita_pericol"]:
                            tinta_x = int(fact[2])  # Preluam destinatia X
                            break
    ...
    deseneaza_text_conturat(ecran, f"Saci: {len(lista_saci_brate)}/5", font_hud, (255, 255, 0), (0, 0, 0), LATIME - 220,
                            20)
    deseneaza_text_conturat(ecran, f"Scapati: {saci_scapati}/{MAX_SACI_SCAPATI}", font_hud, (255, 100, 100), (0, 0, 0),
                            LATIME - 250, 60)
    ...
    deseneaza_text_conturat(ecran, "ENERGY", pygame.font.SysFont(nume_font, 18, bold=True), (255, 255, 255), (0, 0, 0),
                            x_bara + 60, y_bara - 3)
    ...
    if game_over:
        ...
        if motiv_game_over == "concediat":
            deseneaza_text_conturat(ecran, "AI FOST CONCEDIAT!", font_mare, (255, 50, 50), (0, 0, 0), LATIME // 2,
                                    INALTIME // 2 - 40, center=True)
            deseneaza_text_conturat(ecran, "Ai stricat prea multa marfa...", font_hud, (255, 200, 50), (0, 0, 0),
