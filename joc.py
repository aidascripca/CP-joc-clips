...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_p or event.key == pygame.K_ESCAPE:
                if stare_curenta == "JOC": stare_curenta = "PAUZA"
                elif stare_curenta == "PAUZA": stare_curenta = "JOC"
    ...
    if stare_curenta == "MENIU_PRINCIPAL":
        ...
        ui.deseneaza_text_conturat(ecran, "BEAN COUNTERS", font_mare, (255, 215, 0), (0, 0, 0), LATIME // 2, INALTIME // 2 - 100, center=True)
        ui.deseneaza_text_conturat(ecran, "AI Agent Edition", font_hud, (255, 255, 255), (0, 0, 0), LATIME // 2, INALTIME // 2 - 50, center=True)

        if ui.deseneaza_buton(ecran, "Start Joc Normal", font_mic, LATIME // 2 - 125, INALTIME // 2 + 20, 250, 50, (50, 150, 50), (100, 200, 100)) and click_debounce == 0:
            resetare_joc(nivel_start=1, infinit=False)
            click_debounce = 15
            
        if ui.deseneaza_buton(ecran, "Selecteaza Nivel", font_mic, LATIME // 2 - 125, INALTIME // 2 + 90, 250, 50, (50, 50, 150), (100, 100, 200)) and click_debounce == 0:
    ...
    # ==========================================
        ...
        ui.deseneaza_text_conturat(ecran, "Mod Debug (Joaca la infinit)", font_mare, (255, 215, 0), (0, 0, 0), LATIME // 2, 100, center=True)

        for i in range(1, 6):
            if ui.deseneaza_buton(ecran, f"Nivelul {i}", font_mic, LATIME // 2 - 100, 150 + i * 60, 200, 40, (150, 100, 50), (200, 150, 100)) and click_debounce == 0:
                resetare_joc(nivel_start=i, infinit=True)
                click_debounce = 15
                
        if ui.deseneaza_buton(ecran, "Inapoi", font_mic, 20, 20, 120, 40, (150, 50, 50), (200, 100, 100)) and click_debounce == 0:
            stare_curenta = "MENIU_PRINCIPAL"
            click_debounce = 15
    ...
    # ==========================================
        ...
        if stare_curenta == "JOC":
            if timer_tranzitie_nivel > 0:
                ...
                greutate_extra = sum(0.3 if s == "sac_gold" else (0.5 if s == "sac_diamond" else 0) for s in lista_saci_brate)
                ...
                if stamina_epuizata:
                    ...
                    penalizare_greutate = sum(1 if s == "sac_gold" else (2.5 if s == "sac_diamond" else 0) for s in lista_saci_brate)
                ...
                if cooldown_timer == 0:
                    env.reset()
                    env.assert_string(f"(Agent (coordonata_agent {int(pinguin_x)}) (viteza {int(viteza_curenta_pinguin)}) (nr_saci {len(lista_saci_brate)}))")

                    for obj in lista_obiecte:
                        pericol = 1 if obj["tip"] in ["nicovala", "peste"] else 0
                        valoare = 10 if obj["tip"] == "sac" else (30 if obj["tip"] == "sac_gold" else (50 if obj["tip"] == "sac_diamond" else 0))
                ...
                if timer_generare >= interval_generare:
                    ...
                    if random.randint(1, 100) <= prob_obstacol:
                        ...
                        if sans <= 70: tip_ales, gravitatie_obiect = "sac", 0.4
                        elif sans <= 90: tip_ales, gravitatie_obiect = "sac_gold", 0.45
                        else: tip_ales, gravitatie_obiect = "sac_diamond", 0.55

                    lista_obiecte.append({"x": start_x, "y": start_y, "tip": tip_ales, "vx": v_x, "vy": v_y, "unghi": 0, "grav": gravitatie_obiect})
                    timer_generare = 0
                    ...
                    if cooldown_timer == 0 and hitbox_pinguin.colliderect(h_obj):
                        ...
                        if "sac" in obj["tip"]:
                            ...
                            if len(lista_saci_brate) >= 6:
                                vieti -= 1; lista_saci_brate.clear(); cooldown_timer = COOLDOWN_DURATA; tip_cooldown = 7
                                if vieti <= 0: stare_curenta, motiv_game_over = "GAME_OVER", "strivit"
                        elif obj["tip"] == "nicovala":
                            vieti -= 1; lista_saci_brate.clear(); cooldown_timer = COOLDOWN_DURATA; tip_cooldown = 8
                            if vieti <= 0: stare_curenta, motiv_game_over = "GAME_OVER", "strivit"
                        elif obj["tip"] == "peste":
                            vieti -= 1; lista_saci_brate.clear(); cooldown_timer = COOLDOWN_DURATA; tip_cooldown = 9
                            if vieti <= 0: stare_curenta, motiv_game_over = "GAME_OVER", "strivit"
                        ...
                        lista_efecte_sol.append({"x": obj["x"], "y": NIVEL_SOL, "img": img_efect, "timer": 90, "alpha": 255})
        ...
        ui.deseneaza_text_conturat(ecran, titlu_nivel, font_hud, (255, 255, 255), (0, 0, 0), LATIME // 2, 20, center=True)
        ui.deseneaza_text_conturat(ecran, f"SCORE: {scor}", font_hud, (255, 255, 255), (0, 0, 0), 30, 100)
        ui.deseneaza_text_conturat(ecran, f"Saci: {len(lista_saci_brate)}/5", font_hud, (255, 255, 0), (0, 0, 0), LATIME - 220, 20)
        ui.deseneaza_text_conturat(ecran, f"Scapati: {saci_scapati}/{MAX_SACI_SCAPATI}", font_hud, (255, 100, 100), (0, 0, 0), LATIME - 250, 60)
        
        # Buton Pauza Mic Rotund
        if ui.deseneaza_buton_rotund(ecran, "P", font_mic, LATIME - 20, 20, 20, (100, 100, 100), (150, 150, 150)) and click_debounce == 0:
            if stare_curenta == "JOC": stare_curenta = "PAUZA"
            elif stare_curenta == "PAUZA": stare_curenta = "JOC"
            click_debounce = 15
        ...
        ui.deseneaza_text_conturat(ecran, "ENERGY", pygame.font.SysFont("comicsansms", 18, bold=True), (255, 255, 255), (0, 0, 0), x_bara + 60, y_bara - 3)

        if stare_curenta == "JOC" and cooldown_timer > 0:
            if tip_cooldown == 8: ui.deseneaza_text_conturat(ecran, "ZBANG!!", font_efecte, (255, 50, 50), (0, 0, 0), pinguin_x + 60, pinguin_y - 40, center=True)
            elif tip_cooldown == 9: ui.deseneaza_text_conturat(ecran, "PLEOSC!!", font_efecte, (100, 255, 255), (0, 0, 0), pinguin_x + 60, pinguin_y - 40, center=True)
            elif tip_cooldown == 7: ui.deseneaza_text_conturat(ecran, "STUF!!", font_efecte, (255, 200, 50), (0, 0, 0), pinguin_x + 60, pinguin_y - 40, center=True)
            
            if cooldown_timer > 90: text_cd = "Mai incearca!"
            elif cooldown_timer > 60: text_cd = "3"
            elif cooldown_timer > 30: text_cd = "2"
            else: text_cd = "1"
            ui.deseneaza_text_conturat(ecran, text_cd, font_mare, (255, 255, 255), (0, 0, 0), LATIME // 2, INALTIME // 2 - 50, center=True)
        ...
        if stare_curenta == "JOC" and timer_tranzitie_nivel > 0:
            ...
            ui.deseneaza_text_conturat(ecran, f"NIVELUL {nivel}", font_mare, (255, 215, 0), (0, 0, 0), LATIME // 2, INALTIME // 2, center=True)
        ...
        if stare_curenta == "GAME_OVER":
            ...
            if motiv_game_over == "concediat":
                ui.deseneaza_text_conturat(ecran, "AI FOST CONCEDIAT!", font_mare, (255, 50, 50), (0, 0, 0), LATIME // 2, INALTIME // 2 - 40, center=True)
                ui.deseneaza_text_conturat(ecran, "Ai stricat prea multa marfa...", font_hud, (255, 200, 50), (0, 0, 0), LATIME // 2, INALTIME // 2 + 30, center=True)
            else:
                ui.deseneaza_text_conturat(ecran, "GAME OVER!", font_mare, (255, 50, 50), (0, 0, 0), LATIME // 2, INALTIME // 2 - 30, center=True)

            ui.deseneaza_text_conturat(ecran, "Apasa R pentru Restart", font_hud, (255, 255, 255), (0, 0, 0), LATIME // 2, INALTIME // 2 + 80, center=True)
            
            if ui.deseneaza_buton(ecran, "Meniu Principal", font_mic, LATIME // 2 - 100, INALTIME // 2 + 130, 200, 40, (150, 50, 50), (200, 100, 100)) and click_debounce == 0:
                stare_curenta = "MENIU_PRINCIPAL"
                click_debounce = 15
        ...
        if stare_curenta == "PAUZA":
            ...
            ui.deseneaza_text_conturat(ecran, "PAUZA", font_mare, (255, 255, 255), (0, 0, 0), LATIME // 2, INALTIME // 2 - 50, center=True)
            ui.deseneaza_text_conturat(ecran, "Apasa P pentru a continua", font_hud, (200, 200, 200), (0, 0, 0), LATIME // 2, INALTIME // 2 + 20, center=True)

            if ui.deseneaza_buton(ecran, "Meniu", font_mic, LATIME // 2 - 100, INALTIME // 2 + 80, 200, 40, (150, 50, 50), (200, 100, 100)) and click_debounce == 0:
                stare_curenta = "MENIU_PRINCIPAL"
                click_debounce = 15
